...
`plume-lang-parser-loom` is centered on compilers work in Julia. The technical target is: Create a Julia reference implementation for parser workflows, centered on graph analysis, node-edge fixtures, and cycle and reachability reports.
...
The score is deterministic. The current threshold is 167. Risk has a penalty of 6, latency has a penalty of 3, and weight contributes a bonus of 6.
